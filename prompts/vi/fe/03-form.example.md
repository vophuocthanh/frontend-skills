---
title: Ví dụ — Xây dựng Form (React Hook Form + Zod)
type: example
pairs_with: 03-form.md
---

# 📘 Ví dụ — Xây dựng Form (React Hook Form + Zod)

> Một lượt chạy đầy đủ của [`03-form.md`](03-form.md) trên một tình huống thật.
> Copy khối prompt bên dưới và dán thẳng vào agent.

---

```text
Tuân thủ skill `react-client-mastery`. Xây dựng `InviteMemberForm`.

ĐẶC TẢ
- Các trường: email (email hợp lệ, bắt buộc), role (admin|member|viewer, mặc định: member)
- Đích submit: POST /api/invitations
- Khi thành công: đóng modal + toast + invalidate `invitationKeys.lists()`

REPO CONTEXT: `react-hook-form` + `zodResolver` đã là pattern chuẩn của repo — xem
  `src/features/settings/` để có ví dụ đang chạy, sao chép cách bố trí file của nó. Một wrapper
  `TextField` đã tồn tại tại `src/components/TextField.tsx` và đã forward `ref` sẵn,
  nên `register()` hoạt động xuyên qua nó — hãy dùng nó, đừng viết một input wrapper mới.
  Helper toast nằm ở `src/lib/toast.ts`. Test: Vitest + RTL + msw.

NON-GOALS: Không làm luồng mời hàng loạt / qua CSV. Không cho tạo role tùy biến (ba role là cố định).
...
1. Schema trước — MỘT Zod schema duy nhất là nguồn sự thật.
   ...
   - `export type InviteMemberFormValues = z.infer<typeof InviteMemberFormSchema>` — không bao giờ tự viết tay kiểu này.
...
3. SRP — tách ba mối quan tâm:
   - `InviteMemberForm.schema.ts`  → hợp đồng validation
   - `use-invite-member-form.ts`     → form instance, submit handler, mutation, side effects
   - `InviteMemberForm.tsx`        → chỉ JSX; gọi hook và render các field
...
7. Xử lý lỗi (ba tầng, bắt buộc đủ cả ba):
   ...
   - Lỗi field từ phía server: map ngược trở lại bằng `setError(field, ...)`.
     `POST /api/invitations` trả về 409 kèm `{ field: "email", message }` khi
     địa chỉ đó đã có lời mời đang pending — lỗi đó phải rơi đúng vào trường email.
