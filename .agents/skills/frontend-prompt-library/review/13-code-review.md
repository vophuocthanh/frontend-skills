---
title: Code Review Against the Skills
category: Review
skills: [react-client-mastery, nextjs-server-mastery]
principles: [SRP, OCP, LSP, ISP, DIP]
---

# 🔍 Prompt — Code Review Against the Skills

> Use on a PR, a branch diff, or a folder. The agent reviews as a **senior engineer who will maintain this code**, not as a linter.
> Output is prioritized by blast radius, not by line order.

---

```text
Review {{SCOPE}} against the `react-client-mastery` and `nextjs-server-mastery` skills.
CONTEXT: {{CONTEXT}}
DEPTH: {{DEPTH}}
