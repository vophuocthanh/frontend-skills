---
title: Luồng xác thực đầu-cuối (End-to-End)
category: FE + BE
skills: [nextjs-server-mastery, react-client-mastery]
principles: [SRP, LSP, DIP]
---

# 🔐 Prompt — Luồng xác thực đầu-cuối

> Đăng nhập, session, refresh, đăng xuất, bảo vệ route — xuyên suốt ranh giới FE/BE.
> Hai quy tắc KHÔNG BAO GIỜ thỏa hiệp: **token nằm trong httpOnly cookie**, và **middleware không phải là phân quyền**.
...
2. MIDDLEWARE KHÔNG PHẢI LÀ PHÂN QUYỀN. Nó chỉ là một tối ưu hóa redirect, không hơn.
   - `middleware.ts` có thể redirect người dùng chưa xác thực ra khỏi {{PROTECTED}}.
...
═══ KIẾN TRÚC (SRP + DIP) ═══
  ...
  lib/server/adapters/*.ts      phần hiện thực cho {{PROVIDER}}
...
- DIP: use case đăng nhập phụ thuộc vào `AuthGateway`, không phải vào SDK của {{PROVIDER}}.
...
E. BẢO VỆ ROUTE
   - middleware.ts → redirect cho {{PROTECTED}}.
