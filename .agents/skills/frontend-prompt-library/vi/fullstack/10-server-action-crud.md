---
title: Xây dựng CRUD với Server Actions
category: FE + BE
skills: [nextjs-server-mastery, react-client-mastery]
principles: [SRP, OCP, LSP, ISP, DIP]
---

# ⚡ Prompt — Xây dựng CRUD với Server Actions

> Prompt full-stack chủ lực. Vận dụng **cả năm nguyên lý SOLID** trên server.
> Mô hình tư duy: một Server Action là một **HTTP endpoint công khai**. Ai cũng có thể gọi nó. Hãy đối xử với nó đúng như vậy.

---

```text
Tuân theo `nextjs-server-mastery` (+ `react-client-mastery` cho form).
Hiện thực {{OPERATIONS}} cho {{RESOURCE}} bằng Server Actions.
...
═══ KIẾN TRÚC — mũi tên phụ thuộc luôn hướng VÀO TRONG ═══
  ...
  {{RESOURCE}}Form.tsx ('use client', useActionState)
        ↓ gọi
  actions/{{RESOURCE}}-actions.ts   ← MỎNG. auth → validate → ủy thác → revalidate
        ↓ gọi
  lib/server/use-cases/*.ts         ← quy tắc nghiệp vụ. Không biết gì về HTTP/FormData.
        ↓ phụ thuộc vào
  lib/server/ports/*.ts             ← INTERFACE do bạn sở hữu ({{RESOURCE}}Repository, Mailer)
        ↑ được hiện thực bởi
...
2. BẢO MẬT — một Server Action là một ENDPOINT CÔNG KHAI:
   ...
   - Phân quyền theo {{AUTHZ}} được kiểm tra dựa trên chính TÀI NGUYÊN, không chỉ dựa trên role
     (một người dùng đã đăng nhập không được phép cập nhật bản ghi của người khác bằng cách
...
5. DIP — use case phụ thuộc vào PORT, KHÔNG BAO GIỜ vào Prisma/Resend/Stripe:
     export async function createUserUseCase(
       ...
       deps = { userRepo, mailer, audit },   // được tiêm vào → unit-test được với fake
     ) { ... }
   `import 'server-only'` trên mọi port, use case và adapter.

6. LSP cho repository — bản fake và bản thật phải thất bại Y HỆT nhau:
     findById(id): Promise<User | null>   // null khi không tìm thấy — KHÔNG BAO GIỜ throw
     create(input): Promise<User>         // throw ConflictError khi trùng lặp
...
8. SIDE EFFECT ({{SIDE_EFFECTS}}) nằm trong use case, phía sau port của riêng chúng
...
10. CACHE — mọi mutation đều revalidate {{REVALIDATE}}. Quên điều này là bug Server Action
...
═══ SẢN PHẨM BÀN GIAO ═══
  src/shared/api/{{RESOURCE}}/{{RESOURCE}}-schema.ts   (dùng chung FE+BE)
