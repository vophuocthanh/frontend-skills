---
title: Ví dụ — Review Code Đối Chiếu Với Bộ Skills
type: example
pairs_with: 13-code-review.md
---

# 📘 Ví dụ — Review Code Đối Chiếu Với Bộ Skills

> Một lượt chạy đầy đủ của [`13-code-review.md`](13-code-review.md) trên một tình huống thật.
> Copy khối prompt bên dưới và dán thẳng vào agent.

---

```text
Review `git diff develop...HEAD` (PR #142 — `feat(team): invite + revoke teammates`) đối chiếu với các skill `react-client-mastery` và `nextjs-server-mastery`.
CONTEXT: tính năng Team Invitations — Server Actions + page RSC `/team` + danh sách lời mời phía client; nó động tới authorization và RSC boundary, và là tính năng đầu tiên trong codebase này dùng Server Actions
DEPTH: audit toàn diện — các vấn đề chặn merge trước tiên

REPO CONTEXT: PR #142 là tính năng ĐẦU TIÊN trong repo này dùng Server Actions — mọi tính năng
  khác vẫn dùng REST + TanStack Query thông qua `src/lib/http-client.ts`. Các convention
  cần dùng để đánh giá nó là những gì tính năng billing đã thiết lập: ports/adapters +
  `container.ts` dưới `lib/server/`, contract `ActionResult<T>` trong
  `src/shared/api/action-result.ts`, các lỗi có kiểu trong `src/shared/api/errors.ts`, và
  phân tầng api/hooks/components của `src/features/*`.

NON-GOALS: KHÔNG sửa bất cứ thứ gì — chỉ báo cáo. KHÔNG review phần refactor billing không liên quan
  tình cờ nằm chung trong branch này.
